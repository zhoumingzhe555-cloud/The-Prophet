...
if 'manual_te' not in st.session_state: st.session_state.manual_te = None  # 一码中特专享单选变量
...
if not st.session_state.logged_in_user:
        ...
        if st.button("🔥 立即重载并登录验证"):
            if login_user in global_db["users"] and global_db["users"][login_user]["password"] == login_pwd:
                if global_db["users"][login_user].get("status") == "active" or login_user == "admin":
                    ...
                    st.success(f"🎉 登录成功！欢迎进入控制盘")
                    st.rerun()
                else:
                    st.error("❌ 该账号尚未通过管理员审核！")
            else:
                st.error("❌ 用户名或密码错误！")
    ...
    if st.session_state.logged_in_user == "admin":
        with st.sidebar.expander("🛠️ 至尊顶层全盘控制后台", expanded=True):
            ...
            for u in pending_regs:
                if st.button(f"✅ 批准新玩家: {u}", key=f"app_{u}"):
            ...
            for idx, dep in enumerate(pending_deps):
                if st.button(f"🪙 确认 {dep['user']} 进账 ¥{dep['amount']}", key=f"dep_{idx}"):
    ...
    # ----------------- 🎮 核心模拟下注交互区 -----------------
    st.session_state.current_tab = st.radio("切换下注模拟玩法", ["自选平特", "🔥 一码中特", "胆拖组合玩法", "财务充值中心"], horizontal=True)

    # 1. 自选平特玩法
    if st.session_state.current_tab == "自选平特":
        st.subheader("🔮 49码平特多选矩阵")
        ...
        st.info(f"已选平特号码：{sorted(selected_balls)}")
        ...
        if st.button("🔥 确认提交平特模拟注单"):
            ...
            if global_db["users"][u_name]["wallet"] < bet_amt:
                ...
                global_db["bet_history"].append({"user": u_name, "type": "自选平特", "balls": list(selected_balls), "amount": bet_amt, "time": datetime.now().strftime("%H:%M:%S")})
                st.success("🎯 模拟平特下注成功！")
    ...
    # 2. 🔥【新增细化：一码中特玩法】
        ...
        st.caption("⚡ 提示：一码中特为单选玩法。选中黄金球直接锁定，再次点击可取消。")
        
        te_selected = st.session_state.manual_te
        ...
        for i in range(0, 49, 7):
            ...
            for j in range(7):
                num = i + j + 1
                if num <= 49:
                    is_selected = (num == te_selected)
                    color_cls = "ball-s" if is_selected else get_ball_color_class(num)
                    with cols[j]:
                        st.markdown(f"<div class='num-ball-wrap {color_cls}'>", unsafe_allow_html=True)
                        if st.button(f"{num:02d}", key=f"btn_te_{num}"):
                            if te_selected == num:
                                st.session_state.manual_te = None  # 取消选择
                            else:
                                st.session_state.manual_te = num   # 锁定当前球
                            st.rerun()
                        st.markdown("</div>", unsafe_allow_html=True)
        ...
        if st.session_state.manual_te:
            ...
            with col_b1:
                te_amt = st.number_input("中特模拟下单金额 (¥)", min_value=20.0, step=20.0, value=100.0, key="amt_te")
            with col_b2:
                st.metric("🎯 模拟胜率回报评估", f"¥ {te_amt * 42:,.2f}", delta="预估回报 (42倍)")
                
            if st.button("🚀 封单确认：发射中特注单"):
                u_name = st.session_state.logged_in_user
                if global_db["users"][u_name]["wallet"] < te_amt:
                    st.error("❌ 物理钱包余额不足以支付当前特码注单！")
                else:
                    global_db["users"][u_name]["wallet"] -= te_amt
                    global_db["bet_history"].append({
                        "user": u_name, "type": "一码中特", "balls": [st.session_state.manual_te], "amount": te_amt, "time": datetime.now().strftime("%H:%M:%S")
                    })
                    st.success(f"🎉 狙击成功！[{st.session_state.manual_te:02d}] 模拟注单已扣款并打入大盘总线！")
                    st.session_state.manual_te = None
                    st.rerun()
        else:
            st.info("💡 请在上方的 49 码高光数字矩阵中，点击选中您要狙击的目标特码。")
